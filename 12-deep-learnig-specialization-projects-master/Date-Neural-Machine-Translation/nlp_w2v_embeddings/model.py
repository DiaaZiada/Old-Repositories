# -*- coding: utf-8 -*-
"""
Created on Thu Aug  9 22:16:36 2018
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = (np.dot(mu,g)/np.linalg.norm(g)**2)*g
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (np.dot(e_w1,g)/np.linalg.norm(g)**2)*g
    e_w2B = (np.dot(e_w2,g)/np.linalg.norm(g)**2)*g
